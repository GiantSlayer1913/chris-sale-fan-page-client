...
var onGetComments = function onGetComments(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.getComments(data).then(ui.getCommentsSuccess).catch(ui.getCommentsFailure);
};

var onHideComments = function onHideComments(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.hideComments(data).then(ui.hideCommentsSuccess).catch(ui.hideCommentsFailure);
};

var onUpdateComment = function onUpdateComment(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.updateComment(data).then(ui.updateCommentSuccess).catch(ui.updateCommentFailure);
};

var onDeleteComment = function onDeleteComment(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.deleteComment(data).then(ui.deleteCommentSuccess).catch(ui.deleteCommentFailure);
};

var onPreviousSlide = function onPreviousSlide(event) {
  event.preventDefault();
  ui.slideShowBackward();
};
var onNextSlide = function onNextSlide(event) {
  event.preventDefault();
  ui.slideShowForward();
};
...
var hideComments = function hideComments(data) {
  return $.ajax({
    url: config.apiUrl + '/comments',
    method: 'GET',
    headers: {
...
var apiUrl = void 0;
var apiUrls = {
  production: 'https://chris-sale-fan-page-api.herokuapp.com',
...
var signOutSuccess = function signOutSuccess(data) {
  ...
  $('#get-comments').css('display', 'none');
  $('#hide-comments').css('display', 'none');
  $('#update-comment').css('display', 'none');
  $('#destroy-comment').css('display', 'none');
...
// Get comments
var getCommentsSuccess = function getCommentsSuccess(data) {
  ...
  if (store.comments.length > 0) {
    ...
    for (var i = 0; i < store.comments.length; i++) {
      var commentText = store.comments[i].text;
      var commentEmail = store.comments[i].user.email;
      var commentId = store.comments[i].id;
...
var getCommentsFailure = function getCommentsFailure() {
...
// Hide Comments
var hideCommentsSuccess = function hideCommentsSuccess(data) {
  $('#commentsIndiv').css('display', 'none');
...
var hideCommentsFailure = function hideCommentsFailure() {
...
var updateCommentFailure = function updateCommentFailure() {
...
var deleteCommentFailure = function deleteCommentFailure() {
...
var slideShowForward = function slideShowForward() {
  plusSlides(1);
};

var slideShowBackward = function slideShowBackward() {
  plusSlides(-1);
};
...
module.exports = {
  ...
  createCommentFailure: createCommentFailure,
  getCommentsSuccess: getCommentsSuccess,
  getCommentsFailure: getCommentsFailure,
  hideCommentsSuccess: hideCommentsSuccess,
  hideCommentsFailure: hideCommentsFailure,
  updateCommentSuccess: updateCommentSuccess,
  updateCommentFailure: updateCommentFailure,
